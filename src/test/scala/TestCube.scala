...
class TestCube extends AnyFlatSpec with should.Matchers {
  "Volume" should "diff" in {
    def test(v: Cube[Int], x: Pos[Int]): Unit = {
      val d = v diff Cube.unit(x)
    ...
    val v = Cube(Pos(0,0,0), Pos(2,2,2))
  ...
  "Volume" should "alter" in {
    val v = Cube(Pos(0,0), Pos(2,2))
    val rowIter = v.alter(0, 1, 1)
    rowIter should be (Cube(Pos(1,0),Pos(1,2)))
